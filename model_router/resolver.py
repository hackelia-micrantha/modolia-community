...
def _validate_inputs(
    ...
    if not isinstance(request.get("timestamp"), str) or not request["timestamp"]:
        raise ResolutionError(
            "request.timestamp is required for replayable RouteDecision output"
        )
    ...
    if overlap:
        raise ResolutionError(
            f"allowed_sources and forbidden_sources overlap: {sorted(overlap)}"
        )
    unknown_references = (allowed | forbidden) - known
    if unknown_references:
        raise ResolutionError(
            f"RouteConstraints references unknown sources: {sorted(unknown_references)}"
        )
    ...
    if isinstance(preferred, list):
        ...
        if unknown_preferences:
            raise ResolutionError(
                f"RouteRequest preferred_sources references unknown sources: {sorted(unknown_preferences)}"
            )
...
def _verified_evidence(constraints: dict[str, Any], requirement: str) -> bool:
    ...
    return (
        ...
        and bool(fact["reference"].strip())
    )


def _evidence_rejection(requirement: str) -> tuple[str, str]:
    if requirement == "approval":
        return "missing_approval_gate", "required approval evidence is not verified"
    if requirement == "secret_scan":
        return "secret_scan_failed", "required secret-scan evidence is not verified"
    if requirement == "data_classification":
        return (
            "unknown_classification",
            "required data-classification evidence is not verified",
        )
    if requirement == "residency_review":
        return (
            "residency_violation",
            "required residency-review evidence is not verified",
        )
    if requirement == "budget_review":
        return (
            "budget_hard_limit",
            "required budget-review evidence is not verified",
        )
    return (
        "evidence_requirement_unsatisfied",
        f"required {requirement!r} evidence is not verified",
    )
...
def _estimated_cost_usd(source: dict[str, Any], request: dict[str, Any]) -> float | None:
    ...
    if not isinstance(estimate, dict) or not isinstance(pricing, dict):
        return None
...
def _source_rejections(
    ...
    """Return hard rejection reasons using staged, short-circuit evaluation."""
    ...
    # Stage 1: source availability and explicit host source restrictions.
    ...
    if allowed_sources is not None and source_id not in set(allowed_sources):
        _add_rejection(
            details,
            "source_not_allowed",
            "source is outside the explicit allowed_sources set",
        )
    ...
    # Stage 2: privacy, remote-execution, and residency boundary.
    if boundary not in PRIVACY_BOUNDARIES.get(privacy, {"local"}):
        _add_rejection(
            details,
            "privacy_violation",
            f"source boundary {boundary!r} is not allowed for privacy class {privacy!r}",
        )
    if is_remote and constraints.get("remote_allowed") is not True:
        _add_rejection(
            details,
            "remote_execution_forbidden",
            "RouteConstraints forbid remote execution",
        )
    ...
    if residency_requirement == "local_required" and residency != "local":
        _add_rejection(
            details,
            "residency_violation",
            "local_required excludes non-local residency",
        )
    if (
        residency_requirement == "ca_required_when_remote"
        and is_remote
        and residency != "ca"
    ):
        _add_rejection(
            details,
            "residency_violation",
            "CA-required remote execution excludes non-Canadian residency",
        )
    if details:
        return details

    # Stage 3: evidence required by the host for this request, regardless of source.
    ...
    # Stage 4: source-declared remote-use gates and source-specific evidence.
    if is_remote:
        ...
        if isinstance(remote, dict):
            if remote.get("allowed") is not True:
                _add_rejection(
                    details,
                    "source_remote_disabled",
                    "source registry disables remote use of this source",
                )
            ...
            if allowed_privacy and privacy not in allowed_privacy:
                _add_rejection(
                    details,
                    "privacy_violation",
                    f"source remote allowance does not permit privacy class {privacy!r}",
                )
            if details:
                return details
    ...
    if isinstance(minimum_quality, str):
        ...
        if minimum_rank is None or source_rank is None or source_rank < minimum_rank:
            _add_rejection(
                ...
                f"source quality tier {source_quality!r} is below required minimum {minimum_quality!r}",
            )

    estimate = request.get("context_estimate", {})
    if isinstance(estimate, dict):
        requested_tokens = int(estimate.get("input_tokens", 0)) + int(
            estimate.get("max_output_tokens", 0)
        )
        source_context = source.get("context_tokens")
        if requested_tokens and (
            not isinstance(source_context, int) or requested_tokens > source_context
        ):
            _add_rejection(
                details,
                "context_capacity_exceeded",
                f"estimated context {requested_tokens} exceeds provable source capacity {source_context!r}",
            )
    if details:
        return details

    # Stage 6: hard budget. Unknown or non-versioned pricing fails closed for a
    # numeric ceiling; static cost classes remain usable without live pricing.
    ...
    if isinstance(hard_budget, dict):
        ...
        if (
            isinstance(allowed_cost_classes, list)
            and source.get("cost_class") not in set(allowed_cost_classes)
        ):
            _add_rejection(
                details,
                "budget_hard_limit",
                f"source cost class {source.get('cost_class')!r} is outside the allowed hard-budget classes",
            )
        ...
        if isinstance(max_cost, (int, float)):
            ...
            if estimate_usd is None:
                _add_rejection(
                    details,
                    "budget_hard_limit",
                    "source cost cannot be proven within max_cost_usd from versioned inputs",
                )
            elif estimate_usd > float(max_cost):
                _add_rejection(
                    details,
                    "budget_hard_limit",
                    f"estimated source cost {estimate_usd:.6f} USD exceeds max_cost_usd {float(max_cost):.6f}",
                )
...
def _ranking_key(
    ...
    preferred = request.get("preferred_sources", [])
    preferred_rank = (
        preferred.index(source_id)
        if isinstance(preferred, list) and source_id in preferred
        else 9999
    )

    # Host-established residency preference outranks caller/task soft preferences.
    residency_rank = 0
    ...
    return (
        residency_rank,
        0 if preferred_rank != 9999 else 1,
        preferred_rank,
        cost_rank,
        registry_index,
    )
...
def _denial(rejection_details: dict[str, list[dict[str, str]]]) -> list[dict[str, str]]:
    ...
    return [
        {
            "code": "safety_policy_denial",
            "reason": DENIAL_REASON["safety_policy_denial"],
        }
    ]


def _fallback_policy(
    constraints: dict[str, Any], ranked_sources: list[str]
) -> dict[str, list[str]]:
    configured = constraints.get("operational_fallback")
    if not isinstance(configured, dict):
        return {
            "allowed_reasons": [],
            "forbidden_reasons": list(DEFAULT_FORBIDDEN_FALLBACK_REASONS),
        }

    # Fallback cannot occur without at least two eligible sources, even if the host
    # permits operational fallback reasons.
    allowed_reasons = (
        list(configured.get("allowed_reasons", [])) if len(ranked_sources) > 1 else []
    )
    forbidden_reasons = list(
        configured.get("forbidden_reasons", DEFAULT_FORBIDDEN_FALLBACK_REASONS)
    )
    return {
...
def resolve(
    ...
    ranked = sorted(
        eligible,
        key=lambda source_id: _ranking_key(
            source_id,
            sources[source_id],
            request,
            constraints,
            registry_rank[source_id],
        ),
    )
    selected = ranked[0] if ranked else None

    rejected_sources = {
        source_id: [item["reason"] for item in details]
        for source_id, details in rejection_details.items()
    }
    ...
    if selected and constraints.get("remote_allowed") is True:
        decision["remote_allowance_reasons"] = list(
            constraints.get("remote_allowance_reasons", [])
        )
    ...
    if isinstance(estimate, dict):
        decision["token_estimate"] = {
            "input": int(estimate.get("input_tokens", 0)),
            "max_output": int(estimate.get("max_output_tokens", 0)),
        }
