...
from model_router import RESOLVER_VERSION, ResolutionError, resolve, serialize  # noqa: E402
...
class CliError(ValueError):
    """Raised for invalid files or resolver output."""
...
def package_version() -> str:
    ...
    if not isinstance(project, dict):
        raise CliError("pyproject.toml: missing [project] metadata")
    version = project.get("version")
    if not isinstance(version, str) or not version:
        raise CliError("pyproject.toml: project.version must be a non-empty string")
    return version
...
def assert_valid(
    instance: dict[str, Any], schema_path: Path, label: str, *, request: bool = False
) -> None:
    errors = sorted(
        validator(schema_path, request=request).iter_errors(instance),
...
def main() -> int:
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"%(prog)s {package_version()} (resolver {RESOLVER_VERSION})",
    )
    ...
    try:
        ...
        if args.check:
            if args.output is None:
                raise CliError("--check requires --output")
            existing = args.output.read_text(encoding="utf-8") if args.output.exists() else ""
            if existing != content:
                raise CliError(f"{args.output}: resolver output is stale or missing")
